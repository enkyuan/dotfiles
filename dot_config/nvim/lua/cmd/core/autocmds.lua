...
autocmd({ "BufNewFile", "BufRead" }, {
	callback = function()
		if vim.g.loadTeBufLine then
			require("cmd.ui.bufline").setup()
		end
	end,
})
autocmd("UIEnter", {
	callback = function()
		if vim.g.loadGTFOTheme then
			dofile(vim.g.themeCache .. "allThemes")
		end
		if vim.g.loadTeStatusLine then
			vim.opt.statusline = "%!v:lua.require('cmd.ui.statusline').setup()"
		end
	end,
})
...
autocmd("BufWritePost", {
	...
	group = augroup("Reload", { clear = true }),
	callback = function(opts)
		...
		if vim.g.loadTeBufLine then
			require("plenary.reload").reload_module("cmd.ui.bufline")
			vim.opt.tabline = "%!v:lua.require('cmd.ui.bufline').getTabline()"
		end
		if vim.g.loadTeStatusLine then
			require("plenary.reload").reload_module("cmd.ui.statusline")
			vim.opt.statusline = "%!v:lua.require('cmd.ui.statusline').setup()"
		end
		if vim.g.loadGTFOTheme then
...
vim.api.nvim_create_user_command("Update", function()
	require("cmd.core.utils").update()
end, {})
vim.api.nvim_create_user_command("CreateCustom", function()
	require("cmd.core.utils").createCustom()	
end, {})
vim.api.nvim_create_user_command("CheckMason", function()
	require("cmd.core.utils").checkMason()
end, {})
vim.api.nvim_create_user_command("Themes", function()
	require("cmd.themes.pick").setup()
end, {})
vim.api.nvim_create_user_command("ToggleTrans", function()
	require("cmd.themes.pick").toggleTransparency()
end, {})
vim.api.nvim_create_user_command("LazyGit", function()
	require("cmd.core.utils").LazyGit()
end, {})
vim.api.nvim_create_user_command("Ranger", function()
	require("cmd.core.utils").Ranger()
end, {})
