...
M.getCurrentTheme = function()
	local check1, default_theme = pcall(require, "cmd.themes.schemes." .. vim.g.GTFOTheme)
	local check2, custom_theme = pcall(require, "custom.themes.schemes." .. vim.g.GTFOTheme)
	if check1 then
		local precent = default_theme.type == "light" and -5 or 3
		default_theme.lighter = require("cmd.core.utils").change_hex_lightness(default_theme.background, precent)
		default_theme.darker = require("cmd.core.utils").change_hex_lightness(default_theme.background, -3)
		return default_theme
	elseif check2 then
		local precent = custom_theme.type == "light" and -5 or 3
		custom_theme.lighter = require("cmd.core.utils").change_hex_lightness(custom_theme.background, precent)
		custom_theme.darker = require("cmd.core.utils").change_hex_lightness(custom_theme.background, -3)
		return custom_theme
	end
end

M.loadTb = function(g)
	g = require("cmd.themes.integrations." .. g)
	return g
end
...
local tableToStr = function(tb)
	...
	if vim.g.transparency then
		local glassy = require("cmd.themes.transparency")
...
local compile = function()
	if not vim.loop.fs_stat(vim.g.themeCache) then
		vim.fn.mkdir(vim.g.themeCache, "p")
			...
			local allThemes = {}
		for _, file in ipairs(vim.fn.readdir(vim.fn.stdpath("config") .. "/lua/cmd/themes/integrations")) do
